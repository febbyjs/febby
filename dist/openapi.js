...
function buildOpenApiRoutes(openApiJson, router, options) {
    ...
    for (const path in paths) {
        ...
        for (const method in routeData) {
            ...
            const middlewareNames = (controllerInfo["x-middlewares"] || "").split(",");
            if (!controllerName) {
                throw new Error(`missing controller definition for route: ${path} , method: ${method}, define controller function with property name "x-controller"`);
            }
            ...
            console.log(`${method} - ${path} - ${controllerInfo["x-controller"]} - ${controllerInfo["x-middleware"]} - ${controllerInfo["operationId"]}`);
        }
    }
    return router;
}
async function processOpenApiSpecFile(openApiJson, febby, options) {
