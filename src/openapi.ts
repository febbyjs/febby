...
// function registerRoute(router: Router, key: string, data: object): Router {
// 	return router;
// }
...
function buildOpenApiRoutes(
	...
	const middlewareMap: Map<string, NextFunction> = new Map();
	...
	for (const path in paths) {
		...
		for (const method in routeData) {
			...
			const controllerName = controllerInfo["x-controller"];
			const middlewareNames = (
				controllerInfo["x-middlewares"] || ""
			).split(",");

			if (!controllerName) {
				throw new Error(
					`missing controller definition for route: ${path} , method: ${method}, define controller function with property name "x-controller"`
				);
			}
			...
			console.log(
				`${method} - ${path} - ${controllerInfo["x-controller"]} - ${controllerInfo["x-middleware"]} - ${controllerInfo["operationId"]}`
			);
		}
	}
...
export async function processOpenApiSpecFile(
	...
	febby.expressApp.set("openApiSpecInfo", {
		version: openApiJson.openapi,
		info: openApiJson.info,
		externalDocs: openApiJson.externalDocs,
	...
	// for (const serviceUrl of data.servers) {
	//     app.use(serviceUrl.url, loadOpenApiRoutes(data.paths))
	// }
}

export async function parseYAMLFile(fileBuffer: string): Promise<string> {
	return yaml.load(fileBuffer) as string;
}
	...
	paths: {
		"/pet": {
			put: {
				...
				"x-middleware": ["middleware1", "middleware2"],
				requestBody: {
