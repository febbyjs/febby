...
class Vanku {
    ...
    constructSwaggerJson() {
        this.swagger.host = this.swagger.host === '0.0.0.0' ? `${this.swagger.host}:${this.febby.getPort()}` : this.swagger.host;
        // tag construction
    ...
    loadRestRoutes() {
        ...
        if (fs.existsSync(path.join(process.env.PWD, '.swagger.json')) && process.env.NODE_ENV === 'development') {
            ...
            subApp.get('/explorer', swaggerUi.setup(swaggerDocument, '', '', '', 'febby_16.ico', '', `${this.swagger.info.title.toUpperCase()} Documentation`));
        }
    ...
    loadCustomRoutes() {
        ...
        customRouter.use((req, res, next) => {
            ...
            //param handling start
            if (rPath.length > 1 && rPath.charAt(rPath.length - 1) === '/')
                rPath = rPath.slice(0, rPath.length - 1);
            if (!self.routes[rPath]) {
                let slash = rPath.split('/');
                let rSlash = [],
                    handler;
                for (let route in self.routes) {
                    ...
                    if (rSlash.length == slash.length) {
                        for (let i = 0; i < rSlash.length; i++) {
                            if (rSlash[i].includes(':') && rSlash[i].indexOf(':') === 0) {
                                let param = rSlash[i].slice(1, rSlash[i].length);
                                req.params[param] = slash[i];
                            }
                        }
                        return self.processMiddlewares(req, res, (err) => {
                            if (err)
                                return next(err);
                            self.routes[handler][m]['handler'](req, res, next, self.dbModels);
                        }, self.routes[handler][m]['middlewares']);
