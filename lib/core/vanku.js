...
class Vanku {
    ...
    constructor(febby, pack) {
        ...
        if (pack) {
            ...
            this.swagJson.basePath += this.febby.config.restBasePath;
    ...
    constructSwaggerJson() {
        ...
        this.swagJson.host = (port == 80 || port == 443) ? this.swagJson.host : `${this.swagJson.host}:${port}`;
        // tag construction
        for (let model in this.febby.models) {
            let pModel = pluralize(model).toString().toLowerCase();
            this.swagJson.tags.push({
                'name': pModel,
                'description': `${pModel} model`
            });
            this.swagJson.paths[`/${pModel}/{${pModel}Id}`] = {};
            this.swagJson.paths[`/${pModel}`] = {};
            let def = {
                'type': 'object',
                'properties': {}
            };
            ...
            for (let schemaProp in schema) {
                ...
                let dataFormatObj = getDataFormatTypes(schema[schemaProp]);
            ...
            this.swagJson.definitions[pModel] = def;
            let methods = this.febby.models[model].methods || {
                crud: true
            };
            if (methods.crud) {
                this.swagJson.paths[`/${pModel}`]['get'] = {};
                this.swagJson.paths[`/${pModel}/{${pModel}Id}`]['get'] = {};
                this.swagJson.paths[`/${pModel}`]['post'] = {};
                this.swagJson.paths[`/${pModel}/{${pModel}Id}`]['put'] = {};
                this.swagJson.paths[`/${pModel}/{${pModel}Id}`]['delete'] = {};
            } else {
            ...
            if (Object(this.swagJson.paths[`/${pModel}`]).length == 0) {
                delete this.swagJson.paths[`/${pModel}`];
            } else {
                for (let pathMethod in this.swagJson.paths[`/${pModel}`]) {
                    this.swagJson.paths[`/${pModel}`][pathMethod] = {
                        'tags': [pModel],
                        'summary': getSummery(pathMethod, pModel),
                        'description': '',
                        // 'operationId': `${pathMethod}${pModel.capitalize(pModel)}`,
                        'consumes': [
                            'application/json'
                        ],
                        'produces': [
                    ...
                    if (pathMethod === 'post') {
                        this.swagJson.paths[`/${pModel}`][pathMethod]['parameters'].push({
                            'in': 'body',
                            'name': 'body',
                            'description': getSummery(pathMethod, pModel),
                            'required': true,
                            'schema': {
                                '$ref': `#/definitions/${pModel}`
                            }
            ...
            if (Object(this.swagJson.paths[`/${pModel}/{${pModel}Id}`]).length == 0) {
                delete this.swagJson.paths[`/${pModel}/{${pModel}Id}`];
            } else {
                for (let pathMethod in this.swagJson.paths[`/${pModel}/{${pModel}Id}`]) {
                    this.swagJson.paths[`/${pModel}/{${pModel}Id}`][pathMethod] = {
                        'tags': [pModel],
                        'summary': getMethodSummery(pathMethod, pModel),
                        'description': '',
                        // 'operationId': `${pathMethod}${pModel.capitalize(pModel)}`,
                        'consumes': [
                            'application/json'
            ...
            // construct method obj
        }

        function getMethodSummery(method, model) {
            let summery;
            switch (method) {
                case 'get':
        ...
        function getSummery(method, model) {
            let summery;
            switch (method) {
                case 'get':
        ...
        function getDataFormatTypes(schemaPropObj) {
        ...
        // create file and access it
        if (fs.existsSync(path.join(process.env.PWD, '.swaggerJson.json'))) {
            fs.unlinkSync(path.join(process.env.PWD, '.swaggerJson.json'));
        }
        fs.writeFile(path.join(process.env.PWD, '.swaggerJson.json'), JSON.stringify(this.swagJson), (e) => {
            if (e) throw e;
        });

        //console.log(JSON.stringify(this.swagJson));
    }
    /**
     * to get mongodb url
