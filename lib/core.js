...
class Febby {
  ...
  route(routeConfig) {
    ...
    register(route.router || this.mainRouter || this.app, route.method, route.path || route.url || '/', mlwrs || [], route.handler);
  }
  /**
   * Register Route list
  ...
  middleware(middleware, router) {
    router = router || this.mainRouter || this.app;
   ...
   * @param {Array} functions - List of final middlewares
   * @example
   * 
   * febby.bootstrap( () => {
   *  console.log('app started');
   ...
   * })
   */
  finalMiddlewares(functions) {
    functions = functions || [];
    if (!Array.isArray(functions)) {
      functions = [functions];
    }
    functions.forEach(fn => this.app.use(validateFn(fn)));
  }
  /**
   * Register Final Handler after bootstrap
   * @param {Function} fn - Represents final Hnadler of application 
   * @example
   * 
   * febby.bootstrap( () => {
   *  console.log('app started');
